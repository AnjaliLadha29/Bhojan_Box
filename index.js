...
console.log("Initial State from Redux Store:", store.getState());
...
let Nav_Bar =
 () => 
    {
        let data = useSelector((store)=>store.cart.items.length);
        let location = useLocation();
        return (
...
let Header = (props) => 
    {
        ...
        else if(location.pathname==="/Dineout" || location.pathname==="/Cart")
        {
            className = "header4";
        }
        else
...
let Card = (props) =>
    {
        ...
        return(
            ...
            <div className={location.pathname==="/Search" ? "card2" : "card1"}>
                ...
                <img src={props.image} title={props.name} alt={props.name}></img>
