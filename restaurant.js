...
import {addtocart,deletefromcart,restaurant_name} from "./cartslice";
...
let Restaurant = () =>
    {
        ...
        useEffect(()=>
            {
                window.scrollTo(0,0);
                dispatch(restaurant_name({restaurant_name: restaurant.name}));
            },[]);
                                ...
                                {restaurant[element].map((element1)=>
                                    (
                                        ...
                                        <div key={name+`${element1.name}`} className="menu-items">
                                            ...
                                            <img src={element1.image} alt={element1.name} title={element1.name}></img>

                                            {count[element1.name]===undefined ?
                                                ...
                                                ()=>
                                                    {
                                                        update((curr)=>
                                                            {
                                                                let update = {...curr};
                                                                if(update[element1.name]===undefined)
                                                                {
                                                                    update[element1.name]=1;
                                                                }
                                                                return update;
                                                            });
                                                        dispatch(addtocart({items: element1.name}));
                                                    }
                                                }>
                                            ...
                                            <div className="add1">
                                                ...
                                                <button className="add2" style={{borderTopLeftRadius: "10px", borderBottomLeftRadius: "10px"}} onClick={()=>
                                                    {
                                                        update((curr)=>
                                                            {
                                                                let update = {...curr};
                                                                if(update[element1.name]>1)
                                                                    update[element1.name]--;
                                                                else if(update[element1.name]==1)
                                                                    delete update[element1.name];
                                                                return update;
                                                            });   
                                                        dispatch(deletefromcart({items: element1.name})); 
                                                    }}>-</button>
                                                
                                                <span>{count[element1.name]}</span>
                                                
                                                <button className="add2" style={{borderTopRightRadius: "10px", borderBottomRightRadius: "10px"}} onClick={()=>
                                                    {
                                                        update((curr)=>
                                                            {
                                                                let update = {...curr};
                                                                let curr1 = update[element1.name];
                                                                curr1++;
                                                                update[element1.name]=curr1;
                                                                return update;
                                                            });
                                                        dispatch(addtocart({items: element1.name}));
                                                    }}>+</button>
